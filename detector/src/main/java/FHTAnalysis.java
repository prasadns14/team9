...
public class FHTAnalysis {
	...
	*/		
	...
	public void writeFingerPrint(File fingerPrintFile, double[][] fingerPrint, int numOfHeaderTrailerBytes, int numOfFilesProcessed) throws Exception {
	...
	public double[][] updateByteFrequency(double[][] byteFrequency, int numOfFilesProcessed, double[][] byteCount, int numOfHeaderTrailerBytes) {
		for (int headerTrailerByteIndex = 0; headerTrailerByteIndex < numOfHeaderTrailerBytes; headerTrailerByteIndex++) {
			for (int byteIndex = 0; byteIndex < maxByteValue; byteIndex++) {
...
public JSONArray getJsonObject(int numOfHeaderTrailerBytes,double[][] byteFrequency){
 ...
 public void writeJsonFile(int numberOfHeaderTrailerBytes, String fileName, JSONArray json)throws IOException {
	 FileWriter jsonFile = new FileWriter(tarDir  + "\\src\\main\\resources\\visualization_files\\"  +fileName + Integer.toString(numberOfHeaderTrailerBytes) +".json");
	...
	public static void main(String[] args) throws Exception {
        ...
        if (args.length < 1) {
            System.out.println("Usage: FHTAnalysis.java <files_dir>");
        } else {
            ...
            for (int numOfHeaderTrailerBytes = 4; numOfHeaderTrailerBytes < 17; numOfHeaderTrailerBytes *= 2) {
                ...
                for (int fileTypesIndex = 0; fileTypesIndex < fileTypes.length; fileTypesIndex++) {
                    ...
                    File fileType = new File(sourceDir + "\\" + fileTypes[fileTypesIndex] + "\\data\\");
                    File[] files = fileType.listFiles();
                    tarDir = new java.io.File(".").getCanonicalPath();
                    ...
                    System.out.println(tarDir + "\\src\\main\\resources\\" + trailerFingerPrintFile + Integer.toString(numOfHeaderTrailerBytes) + ".txt");
                    int num = 0;
                    ...
                    for (File file : files) {
                        ...
                        if (byteFile.length > numOfHeaderTrailerBytes) {
                            ...
                            /* commenting trailer analysis section  since it won't be considered for analysis when file length is less than numOfHeaderTrailerBytes considered.
                            //trailerFingerPrint
                    ...
                    fht.writeJsonFile(numOfHeaderTrailerBytes, headerFingerPrintFile, headerJson);
                    fht.writeJsonFile(numOfHeaderTrailerBytes, trailerFingerPrintFile, trailerJson);
